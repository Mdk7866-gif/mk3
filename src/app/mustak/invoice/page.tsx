...
function MustakInvoicePageContent() {
  ...
  const handleGeneratePdfAndSave = async () => {
    ...
    try {
      ...
      // 2) Generate latest invoice PDF
      const generateResponse = await fetch(
        "/api/mustakgenerateinvoicepdff",
        { method: "GET" }
      );
      ...
      console.error("Invoice error (Mustak):", error);
    } finally {
    ...
    <div className="min-h-screen bg-gray-100 p-4 sm:p-6 md:p-8">
      ...
      <header className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-8">
        <h1 className="text-3xl font-extrabold text-gray-900">
          Create Invoice for Mustak Khan
        </h1>
        <span className="text-sm text-gray-500">
          Fill client details & items, then generate a professional invoice PDF.
        </span>
      </header>
