...
function getQrDataUri(qrAny: unknown): string | null {
  try {
    ...
    const qr =
      typeof qrAny === 'object' && qrAny !== null && ('qrCode' in qrAny || 'qr' in qrAny)
...
function buildVerificationUrl(invoice: Invoice): string | null {
  const rawUrl =
    typeof invoice.verificationUrl === 'string' && invoice.verificationUrl.length > 4
      ? invoice.verificationUrl
      : typeof invoice.verifyUrl === 'string' && invoice.verifyUrl.length > 4
      ? invoice.verifyUrl
      : typeof invoice.qrLink === 'string' && invoice.qrLink.length > 4
      ? invoice.qrLink
      : null;
  ...
  const itemsRows = (invoice.items ?? [])
    .map(
      ...
      <tr>
        ...
        <td style="padding:5px 4px;text-align:right;font-size:11px;border-bottom:1px solid #e6e6e6;">₹${Number(
          item.rate ?? 0,
        ).toFixed(2)}</td>
        <td style="padding:5px 4px;text-align:right;font-size:11px;border-bottom:1px solid #e6e6e6;">₹${Number(
        ...
        <td style="padding:5px 4px;text-align:right;font-size:11px;border-bottom:1px solid #e6e6e6;font-weight:600;">₹${Number(
          item.totalAmount ?? 0,
        ).toFixed(2)}</td>
      </tr>`,
    )
