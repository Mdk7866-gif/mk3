// src/app/api/mustakgeneratequotationdiscountratepdff/route.ts
import fs from 'fs';
...
const DB_NAME = 'mk3';
const COLLECTION_NAME = 'mustakquotationdiscountrate';
...
function getQrDataUri(qrAny: QrInput): string | null {
  try {
    ...
    if (
      ...
      try {
        const filePath = path.isAbsolute(qr)
          ? qr
...
/**
 * Renders the rate cell HTML.
 * - If originalRate === discountRate → plain price, no strikethrough.
 * - If originalRate > discountRate  → strikethrough original, green discount below.
 */
...
function buildDiscountQuotationHtml(quotation: DiscountQuotationDocument): string {
  ...
  loadLocalImageAsDataURI('phonepe-qr.jpg'); // load but not used in layout
  ...
  try {
    ...
    if (!latest.length) {
      return NextResponse.json(
        { message: 'No discount rate quotations found.' },
        { status: 404 },
      );
    }
    ...
    console.error('Error generating discount rate quotation PDF:', error);
