// src/app/mushahid/quotationdiscountrate/page.tsx
"use client";
...
const CreateMushahidQuotationDiscountPageContent: React.FC = () => {
  ...
  const getQuotationPayload = (): QuotationDiscountData | null => {
    if (
      ...
      toast.error(
        ...
        { id: "mushahidQuotationDRError" }
      );
      return null;
    }

    if (!clientData.date) {
      toast.error("Please select quotation date.", { id: "mushahidQuotationDRError" });
      return null;
    }
    ...
    const validItems = quotationItems.filter((item) => {
      ...
      const orig =
        typeof item.originalRate === "string" ? parseFloat(item.originalRate) : item.originalRate;
      const disc =
        typeof item.discountRate === "string" ? parseFloat(item.discountRate) : item.discountRate;
    ...
    if (dateParts.length !== 3) {
      toast.error("Invalid date format.", { id: "mushahidQuotationDRError" });
      return null;
    }
  ...
  const handleGeneratePdfAndSave = async () => {
    ...
    toast.loading("Saving quotation...", { id: "mushahidQuotationDR" });

    try {
      // 1) Save
      const saveResponse = await fetch("/api/mushahidsavequotationdiscountrate", {
        method: "POST",
      ...
      if (!saveResponse.ok) {
        ...
        throw new Error(errorData?.message || "Failed to save quotation to database.");
      }

      const saveResult = await saveResponse.json();
      console.log("Mushahid Quotation (Discount Rate) saved:", saveResult);

      toast.loading("Quotation saved. Generating PDF...", { id: "mushahidQuotationDR" });

      // 2) Generate PDF
      const generateResponse = await fetch("/api/mushahidgeneratequotationdiscountratepdff", {
        method: "GET",
      });
      ...
      toast.success(successText, { id: "mushahidQuotationDR" });
      setMessage(successText);
    } catch (error) {
      const msg =
        error instanceof Error ? error.message : "Failed to save quotation or generate PDF";
      console.error("Error processing mushahid discount quotation:", error);
      toast.error(`Error: ${msg}`, { id: "mushahidQuotationDR" });
    ...
    <div className="min-h-screen bg-gray-100 p-4 sm:p-6 md:p-8">
      ...
      {isProcessing && (
        <div className="fixed top-0 left-0 right-0 z-50">
          <div className="h-1 w-full bg-gradient-to-r from-blue-400 via-purple-500 to-indigo-500 animate-pulse" />
        </div>
      )}

      <header className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-8">
        <h1 className="text-3xl font-extrabold text-gray-900">
          Create Quotation with Discount — Mushahid Khan
        </h1>
      ...
      <main className="max-w-5xl mx-auto space-y-8">
        ...
        <ItemsDetailsQuotationDiscountRate onItemsChange={handleQuotationItemsChange} />

        {/* Summary card */}
        <div className="mt-4 rounded-lg border border-gray-200 bg-white p-4 sm:p-5 shadow-sm">
          <h2 className="text-lg font-semibold text-gray-800 mb-2">Quotation Summary</h2>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-sm text-gray-700">
            ...
            <div className="text-xs sm:text-sm text-gray-600 max-w-md">
              Only valid items (description, quantity, original rate &amp; discount rate all filled)
              will be saved and included in the PDF. If original rate = discount rate, no
              strikethrough is shown.
            </div>
          </div>
        </div>
...
export default function CreateMushahidQuotationDiscountPage() {
  return (
    <PasswordProtection>
      <CreateMushahidQuotationDiscountPageContent />
    </PasswordProtection>
  );
}
